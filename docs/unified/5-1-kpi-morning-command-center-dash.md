# Unified Story 5.1: KPI "Morning Command Center" Dash (Doanh Thu)
...
> Nguon: _bmad-output/implementation-artifacts/5-1-kpi-morning-command-center-dash.md
...
As a Quan ly Tiem / Chu (Owner),
I want thay ngay Bang Dashboard mo Sang (Trang Chu Ops): Bieu Do Doanh Thu, Bao Cao Xu Huong, va Liet Ke Nhanh (5 Giay) So luong Orders Dang Khoa Huong Toi (Pending), Don Hom Nay,
so that khong mat cong Do Tim.

## Acceptance Criteria

1. **Given** Owner dang nhap thanh cong, **When** navigate toi `/owner`, **Then** hien thi Dashboard Command Mode layout voi collapsible sidebar navigation va KPI overview area trong < 2s load time.

2. **Given** Owner dang o Dashboard, **When** page load hoan tat, **Then** hien thi Revenue Metric Cards gom 3 cards: Doanh thu Ngay / Tuan / Thang voi trend arrows (up xanh la / down do) so sanh voi ky truoc.

3. **Given** Owner dang o Dashboard, **When** data loaded, **Then** hien thi Bar Chart truc quan hoa doanh thu theo ngay trong tuan hien tai (7 ngay), voi date range selector cho phep chon Tuan/Thang.

4. **Given** Owner dang o Dashboard, **When** data loaded, **Then** hien thi Order Statistics card: tong so don (Buy vs Rent breakdown), phan theo trang thai (Pending, Confirmed, In Production, Shipped, Delivered).

5. **Given** Owner dang o Dashboard, **When** co don hang dang san xuat voi deadline < 7 ngay, **Then** hien thi Production Alerts section voi danh sach garments can chu y kem countdown badge.

6. **Given** Owner dang o Dashboard, **When** data loaded, **Then** hien thi Appointments Today list: danh sach lich hen hom nay voi ten khach, slot (Sang/Chieu), trang thai.

7. **Given** Owner dang o Dashboard, **When** click vao bat ky KPI card nao, **Then** co the deep-dive (navigate) toi trang chi tiet tuong ung (Orders, Appointments, etc.).

8. **Given** Owner dang o Dashboard, **When** sidebar navigation hien thi, **Then** co day du menu items: Dashboard (active), San pham, Kho hang, Don hang, Lich hen, Khach hang, Nhan vien, Quy tac, voi collapsible behavior tren mobile/tablet.
...
6. **DO NOT** use English labels in UI. All text in Vietnamese: "Doanh thu", "Don hang", "Lich hen", etc.
...
- 2026-03-14: Code review fixes — 8 issues resolved (3 HIGH, 4 MEDIUM, 1 LOW):
  - [H1] Added date range selector (Tuan/Thang) to RevenueChart (AC#3)
...
## Traceability
- Phase 1 Story: N/A
- Phase 2 Story: 5.1 KPI "Morning Command Center" Dash (Doanh Thu)
- Epic: 5
