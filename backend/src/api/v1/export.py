...
async def export_design(
    ...
    stmt = select(DesignDB).where(
        DesignDB.id == design_id, DesignDB.tenant_id == tenant_id
    )
    ...
    if measurement:
        ...
        measurement_request = MeasurementCreateRequest(
            ...
            ha_eo=measurement.ha_eo,
            vong_nach=measurement.vong_nach,
            sleeve_length=measurement.sleeve_length,
            vong_bap_tay=measurement.vong_bap_tay,
    ...
    final_geometry = ExportService.reconstruct_geometry(
        base_geometry, locked_design.deltas
    )

    # Format annotations
    annotations = ExportService.calculate_delta_annotations(
        locked_design.measurement_deltas or []
    )

    if format == ExportFormat.SVG:
        svg_content = ExportService.generate_svg(
            final_geometry, annotations, str(design.id), design.geometry_hash
        )
        return StreamingResponse(
            io.BytesIO(svg_content.encode("utf-8")),
            media_type="image/svg+xml",
            headers={
                "Content-Disposition": f"attachment; filename=blueprint-{design_id}.svg"
            },
        )
    else:
        ...
        return StreamingResponse(
            ...
            headers={
                "Content-Disposition": f"attachment; filename=blueprint-{design_id}.dxf"
            },
        )
