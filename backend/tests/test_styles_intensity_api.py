...
class TestSubmitIntensity:
    ...
    def test_submit_intensity_returns_all_sliders(self) -> None:
        ...
        payload = {
            "pillar_id": "traditional",
            "intensities": [
                {"key": "do_rong_vai", "value": 38.2},
                {"key": "do_om_than", "value": 61.8},
                {"key": "chieu_dai_ao", "value": 50.0},
    ...
    def test_submit_value_above_max_returns_validation_error(self) -> None:
        ...
        payload = {
            "pillar_id": "traditional",
            "intensities": [{"key": "do_rong_vai", "value": 150.0}],  # max is 100
            "sequence_id": 1,
        }
        response = client.post("/api/v1/styles/submit-intensity", json=payload)
        assert response.status_code == 422
        data = response.json()
        assert "do_rong_vai" in str(data)

    def test_submit_value_below_min_returns_validation_error(self) -> None:
        """AC3: Value below min_value returns error."""
        payload = {
            "pillar_id": "traditional",
            "intensities": [{"key": "do_rong_vai", "value": -5.0}],  # min is 0
            "sequence_id": 1,
        }
        response = client.post("/api/v1/styles/submit-intensity", json=payload)
        assert response.status_code == 422

    def test_submit_high_do_om_than_returns_soft_warning(self) -> None:
        """AC3: do_om_than > 85 triggers soft constraint warning."""
        payload = {
            "pillar_id": "traditional",
            "intensities": [{"key": "do_om_than", "value": 90.0}],
            "sequence_id": 3,
        }
        response = client.post("/api/v1/styles/submit-intensity", json=payload)
        assert response.status_code == 200
        ...
        assert warnings[0]["slider_key"] == "do_om_than"
        assert warnings[0]["severity"] == "soft"
        assert "ôm" in warnings[0]["message"].lower() or "vận động" in warnings[0]["message"].lower()

    def test_submit_do_om_than_at_threshold_no_warning(self) -> None:
        """AC3: do_om_than at exactly 85 does NOT trigger warning."""
        payload = {
            "pillar_id": "traditional",
            "intensities": [{"key": "do_om_than", "value": 85.0}],
            "sequence_id": 2,
        }
        response = client.post("/api/v1/styles/submit-intensity", json=payload)
        assert response.status_code == 200
    ...
    def test_submit_low_do_rong_vai_returns_soft_warning(self) -> None:
        """AC3: do_rong_vai < 30 triggers soft constraint warning."""
        payload = {
            "pillar_id": "traditional",
            "intensities": [{"key": "do_rong_vai", "value": 20.0}],
            "sequence_id": 4,
        }
        response = client.post("/api/v1/styles/submit-intensity", json=payload)
        assert response.status_code == 200
        ...
        warnings = data["warnings"]
        assert len(warnings) == 1
        assert warnings[0]["slider_key"] == "do_rong_vai"
        assert warnings[0]["severity"] == "soft"
