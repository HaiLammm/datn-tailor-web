...
1. **Given** Owner đăng nhập thành công, **When** navigate tới `/owner/production`, **Then** hiển thị Bảng Điều Phối Sản Xuất (Production Board) với danh sách tất cả tailor tasks trong tenant, layout Command Mode (dense 8-12px gaps), load < 2s.

2. **Given** Owner đang ở Production Board, **When** page load hoàn tất, **Then** hiển thị Summary Cards ở header: Tổng tasks, Đang chờ (Assigned), Đang làm (In Progress), Hoàn thành (Completed), Quá hạn (Overdue) — với số đếm tổng hợp tất cả thợ may.

3. **Given** Owner đang ở Production Board, **When** bấm nút "Giao việc mới" (hoặc icon + trên danh sách), **Then** mở form/dialog tạo task với: chọn Đơn hàng (dropdown orders `in_production`), chọn Thợ may (dropdown từ danh sách active staff role=Tailor), đặt Deadline, nhập Ghi chú, nhập Tiền công (piece_rate).

4. **Given** Owner đã điền form giao việc, **When** bấm "Giao việc", **Then** DB lưu record `tailor_tasks` mới (assignee_id, deadline, notes, piece_rate), AND tạo in-app notification cho Thợ may "Có Việc Mới Cần Áo Dài", AND hiển thị task mới trong danh sách ngay lập tức (optimistic update).

5. **Given** Owner đang ở Production Board, **When** danh sách tasks hiển thị, **Then** mỗi task row hiển thị: tên khách hàng, tên sản phẩm, tên thợ may được giao, trạng thái (StatusBadge), deadline, countdown còn lại, tiền công. Có thể sort theo deadline/status/thợ may.

6. **Given** Owner đang ở Production Board, **When** có task với deadline đã qua hoặc < 2 ngày còn lại, **Then** hiển thị Warning Countdown Badge đỏ pulse + text "Quá hạn X ngày" hoặc "Còn X ngày", row được highlight.

7. **Given** Owner đang ở Production Board, **When** click vào một task row, **Then** mở detail drawer/modal hiển thị: thông tin đơn hàng đầy đủ, thông tin thợ may, notes, deadline, trạng thái hiện tại, và nút chỉnh sửa (Edit deadline/notes/reassign).

8. **Given** Owner muốn chỉnh sửa task, **When** bấm Edit trong detail view, **Then** có thể cập nhật deadline, notes, piece_rate, hoặc reassign sang thợ may khác. Lưu thành công → cập nhật UI.

9. **Given** Owner đang ở Production Board, **When** sử dụng bộ lọc, **Then** có thể filter tasks theo: Thợ may (dropdown), Trạng thái (assigned/in_progress/completed/overdue), Khoảng deadline (sắp hết hạn/quá hạn).
...
- [x] Task 2: Backend — Notification on Task Assignment (AC: #4)
  - [x] 2.1: Add task assignment notification template to `backend/src/services/notification_creator.py`: `TASK_ASSIGNMENT_MESSAGE = ("Công việc mới được giao", "Bạn được giao công việc mới: {garment_name} - Hạn chót: {deadline}.")`
...
- [x] Task 4: Frontend — Production Board Page & Components (AC: #1, #2, #5, #6, #9)
  ...
  - [x] 4.4: Create `frontend/src/components/client/production/ProductionTaskTable.tsx` — Sortable table of all tasks with columns: Khách hàng, Sản phẩm, Thợ may, Trạng thái, Hạn chót (countdown), Tiền công. Clickable rows open detail.
  - [x] 4.5: Create `frontend/src/components/client/production/ProductionFilters.tsx` — Filter bar: dropdown Thợ may (from staff list), dropdown Trạng thái, toggle Quá hạn/Sắp hết hạn
...
- [x] Task 7: Frontend — Add sidebar menu item & tests (AC: #1)
  - [x] 7.1: Add "Sản xuất" menu item to `WorkplaceSidebar.tsx` Owner section linking to `/owner/production` (with factory/wrench icon)
...
- [ ] Review Follow-ups (AI)
  ...
  - [ ] [AI-Review][Med] Add "sắp hết hạn" filter option to ProductionFilters per AC #9 — `ProductionFilters.tsx`
...
- [ ] [Med] Missing "sắp hết hạn" deadline filter in UI — AC #9 partial gap
...
| `WorkplaceSidebar.tsx` | `frontend/src/components/client/workplace/WorkplaceSidebar.tsx` | ADD "Sản xuất" menu item to Owner section. |
...
|:---|:---|:---|
| > 7 days remaining | Green | "Còn X ngày" |
| 2-7 days remaining | Amber | "Còn X ngày" |
| < 2 days remaining | Red | "Còn X ngày ⚠️" |
| Overdue | Red pulse | "Quá hạn X ngày" |
| No deadline | Gray | "Không có hạn" |
...
7. **DO NOT** use English labels in UI. All text in Vietnamese: "Giao việc", "Sản xuất", "Thợ may", "Hạn chót", "Tiền công", etc.
...
- Add "Sản xuất" to WorkplaceSidebar.tsx Owner menu items (between "Đơn hàng" and "Cho thuê" or at appropriate position).
...
- "Sản xuất" menu item added to WorkplaceSidebar.tsx Owner section
