# Story 6.2: Khai Mở & Chuyển Hóa Khách (Lead-to-Customer Conversion)
...
So that tôi không cần tốn công nhập dữ liệu X2, tiết kiệm nhanh gọn tại cửa hàng.

## Acceptance Criteria

1. **Given** a Lead card with "Hot" classification on the CRM Leads Board
   **When** Owner clicks the "Chuyển khách" (Convert to Customer) action button
...
2. **Given** a Lead with status "Warm" or "Cold"
   ...
   **Then** the "Chuyển khách" button is still available (not restricted to Hot only per re-analysis — Owner may convert any lead at their discretion)
   **And** a warning is shown for non-Hot leads: "Lead chưa ở trạng thái Hot. Bạn có chắc muốn chuyển?"

3. **Given** a Lead whose phone number already exists in `customer_profiles` for this tenant
   **When** Owner attempts to convert
   **Then** the system shows error: "Khách hàng với số điện thoại này đã tồn tại"
...
5. **Given** two Owners viewing the same Lead simultaneously
   ...
   **Then** Owner B's conversion attempt fails gracefully with message: "Lead này đã được chuyển thành khách hàng"
...
- [x] Task 6: Frontend — Convert button and dialog in LeadsBoardClient (AC: #1, #2, #3, #5)
  - [x] 6.1: Add "Chuyển khách" button to each Lead card's Actions column in `LeadsBoardClient.tsx`
  - [x] 6.2: Create conversion confirmation dialog (Radix AlertDialog) showing: Lead name, phone, email, classification badge, checkbox "Tạo tài khoản đăng nhập cho khách" (default unchecked)
  ...
  - [x] 6.6: Success toast with customer name: "Đã chuyển {name} thành khách hàng thành công!"
...
async def convert_lead_to_customer(
    ...
    if not lead:
        raise HTTPException(404, "Lead không tồn tại hoặc đã được chuyển")
    ...
    if existing.scalar_one_or_none():
        raise HTTPException(409, "Khách hàng với số điện thoại này đã tồn tại")
    ...
    customer = CustomerProfileDB(
        ...
        phone=lead.phone or "",
        email=lead.email,
        notes=f"Chuyển từ Lead (source: {lead.source}). {lead.notes or ''}"
    )
...
Error Responses:
- 404: { "error": { "code": "ERR_LEAD_NOT_FOUND", "message": "Lead không tồn tại hoặc đã được chuyển" } }
- 409: { "error": { "code": "ERR_DUPLICATE_PHONE", "message": "Khách hàng với số điện thoại này đã tồn tại" } }
- 403: { "error": { "code": "ERR_FORBIDDEN", "message": "Chỉ Owner mới có quyền chuyển Lead" } }
```
...
- **Button label:** "Chuyển khách" with an arrow-right icon
- **Confirmation dialog:** Radix AlertDialog with:
  ...
  - Checkbox: "Tạo tài khoản đăng nhập cho khách" (unchecked by default, only shown if lead has email)
  - Warning for non-Hot: amber text "Lead chưa ở trạng thái Hot"
  - Confirm button: "Chuyển khách" (Heritage Gold)
  - Cancel button: "Hủy"
- **Row exit animation:** Framer Motion `AnimatePresence` with `motion.tr` — slide-out + fade, duration 300ms
- **Success toast:** Green toast "Đã chuyển {name} thành khách hàng thành công!"
