# Unified Story 1.7: Quan ly ho so so do khach hang

Status: done

## Phase 1 — Requirements (Original)
> Nguồn: docs/1-3-quan-ly-ho-so-so-do.md
...
2. **Customer Profile Form:** Form nhập thông tin khách hàng bao gồm:
   - Avatar (optional, image upload - PNG/JPG, max 5MB)
   - Họ tên (required)
...
6. **Data Validation:**
   - Avatar: PNG/JPG only, max 5MB, stored in cloud storage (S3/Cloudinary)
...
7. **Customer List View:** Hiển thị danh sách khách hàng với:
   - Avatar (thumbnail), Tên, SĐT, Email
...
- [ ] **Backend: Database Schema (AC: 1-10)**
  - [ ] Tạo bảng `customer_profiles` với schema:
    ...
    - `avatar_url` (VARCHAR 500, nullable) - Avatar image URL (stored in cloud storage)
...
### Avatar/Image Handling Strategy

**Frontend Upload:**
- Use `<input type="file" accept="image/png,image/jpeg" />` for avatar upload
- Client-side validation: max 5MB, PNG/JPG only
- Show preview before upload
- Use `FormData` API to send multipart/form-data to backend
- Placeholder avatar if no image provided

**Backend File Storage:**
- Option 1 (Recommended for MVP): Store in local filesystem at `backend/uploads/avatars/{tenant_id}/{customer_id}/`
- Option 2 (Production): S3/Cloudinary bucket with signed URLs (future enhancement)
- Store relative URL in `customer_profiles.avatar_url` database field
- Validate file MIME type server-side (not just extension)
- Generate unique filename to prevent conflicts: `{customer_id}_{timestamp}.{ext}`

**API Endpoint for Avatar Upload:**
```
POST /api/v1/customers/{customer_id}/avatar
...
{
  "data": {
    "avatar_url": "/uploads/avatars/{tenant_id}/{customer_id}/image.jpg",
...
- Invalid MIME type: `422 Unprocessable Entity` - "Chỉ chấp nhận file PNG hoặc JPG"
- Upload failed: `500 Internal Server Error` - Log to monitoring service

**Database:**
- `avatar_url`: VARCHAR(500) - relative path or URL
- Nullable field (avatar is optional)
- On customer delete (soft delete), avatar file remains for recovery period
...
/owner/customers → Click "Thêm khách hàng"
   ↓
/owner/customers/new → Fill form (Avatar + Info + Measurements)
   ↓
   [Avatar] Optional: Upload image (PNG/JPG, max 5MB)
   ↓
   ...
   ↓
Submit → Backend validates all fields → Upload avatar if provided → Create profile (+ user if requested)
   ↓
...
- **Invalid avatar format:** `422 Unprocessable Entity` - "Avatar phải là file PNG hoặc JPG, dung lượng không quá 5MB"
...
- **[Low] Delete Customer UI Button:** Deferred (not critical for MVP, requires auth state management in frontend)
...
## Traceability
- Phase 1 Story: 1.3 Quản lý Hồ sơ & Số đo
- Phase 2 Story: 1.7 Quản lý hồ sơ số đo khách hàng
- Epic: 1
