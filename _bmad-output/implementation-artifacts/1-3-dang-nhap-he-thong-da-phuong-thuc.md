# Unified Story 1.3: Dang nhap he thong da phuong thuc

Status: done

## Phase 1 — Requirements (Original)
> Nguồn: docs/1-1-dang-nhap-he-thong-da-phuong-thuc.md
...
- [x] **Backend: Auth Service & Endpoints (AC: 2, 3, 7)**
  ...
  - [x] Tạo `backend/src/core/security.py` với JWT encode/decode (python-jose), password hashing (passlib/bcrypt)
...
- **Next.js 16 auto-detects `proxy.ts`** — KHÔNG cần register thủ công trong `next.config.ts`. Build output xác nhận `Proxy (Middleware)`.
...
- **Thư viện:** Dùng `python-jose[cryptography]`
...
## Traceability
- Phase 1 Story: 1.1 Đăng nhập Hệ thống Đa phương thức
- Phase 2 Story: 1.3 Đăng nhập hệ thống đa phương thức
- Epic: 1
