...
| **Pattern Generation** (Design Session — Owner) | **Desktop/Tablet-first** | Split-pane layout (measurement input trái, SVG preview phải), precision input cho 10 số đo, export controls |
| **Pattern Viewing** (Task Detail — Tailor) | **Cross-platform** | SVG viewer responsive với zoom/pan, mobile touch-friendly, embedded trong task detail |
...
Tailwind CSS v4 (Design Tokens + Utilities)
    ...
    └── Custom Components (Domain-specific)
        ...
        ├── Measurement Form (10-field auto-fill from customer profile)
        ├── Pattern Preview (SVG viewer with zoom/pan, toggle 3 pieces)
        └── Pattern Export Bar (SVG/G-code buttons + laser speed/power)
```
...
**Owner Pattern Generation Flow:**
1. **Initiation:** Mở Design Session → Chọn customer từ danh sách → Auto-fill 10 số đo từ profile
2. **Interaction:** Review/chỉnh số đo → Tap "Tạo rập" → Split-pane: SVG preview 3 rập (thân trước, thân sau, tay áo)
3. **Feedback:** Preview cập nhật realtime khi chỉnh số đo. Toggle xem từng rập riêng lẻ
4. **Completion:** 2 nút export [Xuất SVG] [Xuất G-code + speed/power] → Đính kèm rập vào Order khi giao việc

**Tailor Pattern Viewing Flow:**
1. **Initiation:** Mở Task Detail → Thấy section "Bản rập đính kèm"
2. **Interaction:** Tap để xem full SVG → Zoom/pan trên cả desktop và mobile touch
3. **Feedback:** Toggle giữa 3 rập, xem geometry params (chiều rộng ngực, eo, mông...)
4. **Completion:** Hiểu rõ bản rập → Bắt đầu sản xuất. Có thể export SVG/G-code từ task detail
...
### 6. Hành trình Cô Lan: "Pattern Generation" (Design Session)

```mermaid
graph TD
    A[Mở Design Session] --> B[Chọn Customer từ danh sách]
    B --> C[Auto-fill 10 số đo từ profile]
    C --> D{Số đo OK?}
    D -- Chỉnh --> E[Sửa số đo trực tiếp trên form]
    E --> D
    D -- OK --> F["Tap: Tạo rập"]
    F --> G[Split-pane: SVG Preview 3 rập]
    G --> H{Xem chi tiết?}
    H -- Toggle --> I[Chọn rập: Thân trước / Thân sau / Tay áo]
    I --> H
    H -- Export --> J{Chọn format}
    J -- SVG --> K[Download SVG 1:1]
    J -- G-code --> L[Chọn speed/power → Download G-code]
    K --> M{Đính kèm Order?}
    L --> M
    M -- Có --> N["Gắn rập vào Order → Giao việc Tailor"]
    M -- Không --> O[Lưu session → Quay lại sau]
```
...
**8. MeasurementForm**
- **Purpose:** Form nhập 10 số đo cơ thể, auto-fill từ customer profile
- **States:** Empty (chọn customer trước), Filled (auto-fill từ profile), Editing (chỉnh tay), Error (validation min/max fail)
- **Content:** 10 fields: độ dài áo, hạ eo, vòng cổ, vòng nách, vòng ngực, vòng eo, vòng mông, độ dài tay, vòng bắp tay, vòng cổ tay
- **Unique:** Customer Combobox search ở đầu form → chọn = auto-fill toàn bộ. Error messages Tiếng Việt chuyên ngành. Đơn vị cm cố định

**9. PatternPreview**
- **Purpose:** SVG viewer hiển thị bản rập kỹ thuật với zoom/pan
- **States:** Loading (skeleton), Rendered (SVG display), Empty (chưa generate)
- **Variants:** Full (Design Session — split-pane phải), Embedded (Task Detail — compact)
- **Unique:** Toggle 3 rập (thân trước/sau/tay áo). Pinch-to-zoom trên mobile touch. Reused ở cả Owner Design Session và Tailor Task Detail (read-only)

**10. PatternExportBar**
- **Purpose:** Thanh export với 2 nút SVG/G-code + laser config
...
- **Phase 4 (Pattern Generation):** MeasurementForm, PatternPreview, PatternExportBar
...
### Tổng kết
- **Revision:** 3 (cập nhật Epic 11 — Technical Pattern Generation)
...
- **Nội dung bao gồm:**
    ...
    - Responsive Design & Accessibility
    - Technical Pattern Generation UX (Epic 11 — Rev 3 addition)

### Revision 3 Changes (2026-04-03)
- **Platform Strategy:** Added Pattern Generation (Desktop/Tablet-first) and Pattern Viewing (Cross-platform)
- **Experience Mechanics:** Added Owner Pattern Generation Flow and Tailor Pattern Viewing Flow
- **Journey Flows:** Added Pattern Generation mermaid diagram
- **Custom Components:** Added MeasurementForm, PatternPreview, PatternExportBar (3 new)
- **Component Phases:** Added Phase 4 (Pattern Generation)

### Hành động tiếp theo
1. **Epics & Stories** — Thêm Epic 11 với 6 stories vào epics.md
2. **Implementation Readiness** — Kiểm tra đồng bộ tất cả artifacts (PRD, Architecture, UX, Epics)
3. **Component Development** — Phase 4 components (MeasurementForm, PatternPreview, PatternExportBar) sau khi Epic 10 hoàn thành
