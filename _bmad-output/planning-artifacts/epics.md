...
FR82-FR90: Epic 10 - Unified Order Workflow (5-Phase)
...
### Epic 10: Quy trình Đơn hàng Toàn diện (Unified Order Workflow)
Hệ thống hỗ trợ quy trình xử lý đơn hàng 5 giai đoạn, phân biệt rõ 3 loại dịch vụ (Mua sẵn / Thuê / Đặt may), bao gồm: xác thực số đo, thanh toán đa hình thức, phê duyệt Owner, sub-steps theo loại dịch vụ, và luồng bàn giao hoàn chỉnh.
...
Hệ thống hỗ trợ quy trình xử lý đơn hàng 5 giai đoạn, phân biệt rõ 3 loại dịch vụ (Mua sẵn / Thuê / Đặt may), bao gồm: xác thực số đo cho đặt may, thanh toán nhiều hình thức (Full / Deposit / Security Deposit), phê duyệt Owner, sub-steps sản xuất theo loại dịch vụ, và luồng bàn giao hoàn chỉnh.
**FRs covered:** FR82, FR83, FR84, FR85, FR86, FR87, FR88, FR89, FR90.
**Added by:** Sprint Change Proposal 2026-03-26.

### Story 10.1: DB Migration & Service Type Model

As a Kiến trúc sư hệ thống,
I want mở rộng Order model với service_type (buy/rent/bespoke), thêm status enum mới, tạo bảng payment_transactions, và các fields deposit/security,
So that hệ thống có nền tảng dữ liệu phục vụ quy trình đơn hàng 3 loại dịch vụ.

**Acceptance Criteria:**

**Given** Database PostgreSQL hiện tại với bảng orders đang hoạt động
**When** Chạy migration 019_unified_order_workflow.sql
**Then** Bảng orders có thêm: service_type (default 'buy'), security_type, security_value, pickup_date, return_date, deposit_amount, remaining_amount
**And** Bảng payment_transactions được tạo với columns: id, tenant_id, order_id, payment_type, amount, method, status, gateway_ref, timestamps
**And** Order status enum mở rộng: pending_measurement, confirmed, preparing, ready_to_ship, ready_for_pickup, renting, returned, completed
**And** Dữ liệu orders cũ không bị ảnh hưởng (default service_type='buy')
...
**Given** Khách chọn "Đặt may" (Bespoke) tại trang sản phẩm hoặc checkout
**When** Hệ thống gọi API `/api/v1/orders/check-measurement` kiểm tra profile số đo
**Then** Trường hợp chưa có số đo: Hiển thị thông báo + CTA "Đặt lịch hẹn đo số đo" chuyển hướng đến Booking Calendar
**And** Trường hợp đã có số đo: Hiển thị bảng thông số kèm thời gian cập nhật gần nhất
**And** Khách có thể "Xác nhận số đo này" để tiếp tục checkout hoặc "Yêu cầu đo lại" để đặt lịch mới

### Story 10.3: Checkout Phân loại Dịch vụ (Service-Type Checkout)

As a Khách hàng,
I want checkout flow phân biệt rõ ràng theo loại dịch vụ tôi chọn (Mua / Thuê / Đặt may) với form thanh toán phù hợp,
So that tôi chỉ điền thông tin cần thiết và thanh toán đúng số tiền cho loại dịch vụ.

**Acceptance Criteria:**

**Given** Khách đã thêm sản phẩm vào giỏ hàng và tiến hành checkout
**When** Hệ thống xác định service_type của từng item
**Then** Mua sẵn: Form thanh toán 100% giá trị, flow checkout hiện tại
**And** Thuê: Form bổ sung ngày nhận/trả, lựa chọn CCCD hoặc tiền cọc thế chân, thanh toán = deposit only
**And** Đặt may: Hiển thị badge "Số đo đã xác nhận", thanh toán = deposit only
**And** Order được tạo với status='pending', payment_type phù hợp (full/deposit)
**And** Payment_transaction record được tạo tương ứng

### Story 10.4: Owner Approve & Auto-routing (Phê duyệt & Điều phối)

As a Chủ tiệm (Owner),
I want phê duyệt đơn hàng trước khi đưa vào sản xuất/chuẩn bị, và hệ thống tự động điều phối đơn đến đúng bộ phận,
So that tôi kiểm soát được chất lượng đầu vào và công việc được phân phối hiệu quả.

**Acceptance Criteria:**

**Given** Order Board hiển thị đơn hàng status='pending' với badge loại dịch vụ (Mua xanh / Thuê vàng / Đặt may tím)
**When** Owner nhấn nút "Phê duyệt" trên đơn hàng
**Then** API `POST /api/v1/orders/{id}/approve` chuyển status pending → confirmed → in_progress
**And** Đơn Đặt may: Tự động tạo TailorTask đính kèm file số đo đã xác nhận, thông báo thợ may
**And** Đơn Thuê/Mua: Chuyển yêu cầu xuống kho (status='preparing')
**And** Toast notification hiển thị routing destination
...
**Given** Đơn hàng Thuê hoặc Mua đã được Owner phê duyệt (status='preparing')
...
**And** Khi sub-step cuối hoàn thành: status chuyển thành ready_to_ship hoặc ready_for_pickup
...
**Given** Đơn hàng có status='ready_to_ship' hoặc 'ready_for_pickup' và còn remaining_amount > 0
**When** Khách thanh toán phần còn lại qua API `POST /api/v1/orders/{id}/pay-remaining`
**Then** Payment_transaction mới được tạo (type='remaining'), remaining_amount = 0
**And** Đơn hàng chuyển status: shipped (giao hàng) hoặc delivered (nhận tại tiệm)
**And** Khi khách xác nhận nhận hàng: status → delivered → completed (cho đơn Mua/Đặt may)
**And** Đơn Mua 100%: Không cần bước remaining payment, chuyển thẳng shipped → delivered → completed

### Story 10.7: Rental Return & Security Refund (Trả đồ thuê & Hoàn cọc)

As a Chủ tiệm,
I want quản lý quy trình trả đồ thuê, kiểm tra tình trạng sản phẩm và hoàn trả cọc/CCCD cho khách,
So that dịch vụ cho thuê khép kín, khách hài lòng và tài sản tiệm được bảo vệ.

**Acceptance Criteria:**

**Given** Đơn thuê đã delivered, status chuyển sang 'renting', hệ thống theo dõi thời gian khách giữ đồ
**When** Khách mang đồ trả tiệm
**Then** Owner kiểm tra tình trạng sản phẩm (Good / Damaged / Lost) trên form trả đồ
**And** Nếu Good: API `POST /api/v1/orders/{id}/refund-security` hoàn trả tiền cọc hoặc CCCD, status → returned → completed
**And** Nếu Damaged/Lost: Owner ghi nhận phí phạt, trừ vào tiền cọc hoặc liên hệ khách, sau đó status → returned → completed
**And** Hệ thống gửi email thông báo hoàn trả cọc/CCCD cho khách
